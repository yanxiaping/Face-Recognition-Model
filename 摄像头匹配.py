...
def extract_face_feature_from_frame(face_img, model):
    """从摄像头帧的人脸区域提取特征（和图片提取逻辑一致）"""
    transform = transforms.Compose([
    ...
    try:
        ...
        img_tensor = transform(face_rgb).unsqueeze(0).to(DEVICE)

        with torch.no_grad():
            feat = model(img_tensor)
        return feat.cpu().numpy().squeeze()
    except Exception as e:
        print(f"❌ 提取摄像头人脸特征失败：{str(e)[:50]}")
        return None
...
# ===================== 摄像头实时匹配（和图片测试同逻辑） =====================
# 在 camera_real_time_match 函数开头，添加平滑缓存变量
# ===================== 摄像头实时匹配（修复版：稳定、不狂Unknown、不跳名） =====================
def camera_real_time_match():
    model = load_encoder()
    db = build_adaptive_db()
    if not db:
        print("❌ 特征库为空，无法进行摄像头匹配")
        return
    ...
    # ============================================================
    # 1) 构建 Prototype 库：每个人用“均值特征（再归一化）”代表
    # ============================================================
    prototype_db = {}
    for name, feats in db.items():
        ...
        m = m / (np.linalg.norm(m) + 1e-9)  # 再归一化
        prototype_db[name] = m

    if len(prototype_db) == 0:
        print("❌ Prototype库为空，无法进行摄像头匹配")
        return

    # ============================================================
    # 2) 稳定器（不会锁死 Unknown）
    #    stable_name：最终显示
    #    candidate_name：当前候选（连续出现N帧才上屏）
    # ============================================================
    stable_name = "Unknown"
    ...
    # ============================================================
    # 3) Unknown 判定阈值（建议先用这组，再按你数据微调）
    #    - ABS_DIST_TH: 绝对距离阈值（越小越严格）
    #    - MARGIN_TH: 第一名与第二名“领先幅度”阈值
    # ============================================================
    ABS_DIST_TH = 1.25   # 你截图best_dist≈1.05，用1.20~1.30都可试
    MARGIN_TH = 0.05     # (second-best)/second 的领先幅度，0.03~0.08可调

    print(f"\n=====================================")
    print("🎥 摄像头实时匹配模式（按q退出）")
    print(f"📌 Prototype库人数：{len(prototype_db)}")
    print(f"📌 ABS_DIST_TH={ABS_DIST_TH} | MARGIN_TH={MARGIN_TH} | STABLE_THRESH={STABLE_THRESH}")
    print("=====================================")
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            face_feat = extract_face_feature_from_frame(face_img, model)
            if face_feat is None:
                # 不更新稳定器，避免抖动
                cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0, 255), 2)
                cv2.putText(frame, "Unknown (0.00%)", (x, y-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                continue

            # ============================================================
            # 4) 用 prototype 做匹配（每人只比一次）
            # ============================================================
            dists = {}
            for name, proto in prototype_db.items():
                dists[name] = float(euclidean_dist(face_feat, proto))

            sorted_res = sorted(dists.items(), key=lambda x: x[1])
            best_name, best_dist = sorted_res[0]
            if len(sorted_res) > 1:
                second_name, second_dist = sorted_res[1]
            else:
                second_name, second_dist = "None", best_dist + 1e-6

            # ============================================================
            # 5) margin-based 置信度（更符合直觉）
            #    margin = (second - best) / second   -> 0~1
            # ============================================================
            margin_conf = (second_dist - best_dist) / (second_dist + 1e-9)
            margin_conf = float(np.clip(margin_conf, 0.0, 1.0))

            # ============================================================
            # 6) Unknown 判定：绝对距离 + 相对领先幅度 双条件
            # ============================================================
            if (best_dist <= ABS_DIST_TH) and (margin_conf >= MARGIN_TH):
                this_name = best_name
                this_conf = margin_conf
            else:
                this_name = "Unknown"
                this_conf = 0.0

            # ============================================================
            # 7) 稳定器：候选连续出现N帧才更新 stable_name
            # ============================================================
            if this_name == candidate_name:
                candidate_count += 1
            else:
                candidate_name = this_name
                candidate_count = 1

            if candidate_count >= STABLE_THRESH:
                stable_name = candidate_name

            show_name = stable_name
            conf_show = this_conf if show_name != "Unknown" else 0.0
            ...
            text = f"{show_name} ({conf_show:.2%})"
            cv2.putText(frame, text, (x, y-10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

            # 终端调试信息（非常关键，用来调阈值）
            print(
                f"\rbest={best_name} dist={best_dist:.3f} | "
                f"2nd={second_name} dist2={second_dist:.3f} | "
                f"margin={margin_conf:.3f} | "
                f"cand={candidate_name}:{candidate_count} stable={stable_name}      ",
                end=""
            )
